...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public Shiba = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); //Shiba Inu (Bep20)
...
contract SHIP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public deadWallet = 0x000000000000000000000000000000000000dEaD; // dead

    address public rewardToken = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // Shiba Inu (Bpe 20)

    uint256 public maxTransactionAmount = 15000000000 * (10**18); // 15,000,000,000 (1.5%)
    ...
    address payable public _marketingWalletAddress = payable(0x69A18f47c171dC5E2636e2B7d368CCf1b2Ae2d6A); // Marketing
    ...
    constructor() ERC20("ShiPonzi Protocol", "SHIP") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SHIP: The dividend tracker already has that address");

        ShibaDividendTracker newDividendTracker = ShibaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SHIP: The new dividend tracker must be owned by the token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SHIP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIP: Account is already the value of 'excluded'");
    ...
    function updateMaxTransactionLimit(uint256 _newLimit) external onlyOwner{
        ...
        require(maxTransactionAmount > totalSupply().div(100), "SHIP: Max transaction limit 1% of total supply");
    }
    ...
    function updateSellTax(uint256 _rewardFee, uint256 _marketingFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner{
        ...
        require(sellTotalFee <= 25, "SHIP: Total tax must be less than or equal to 25 %");
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SHIP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHIP: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHIP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHIP: Cannot update gasForProcessing to same value");
...
contract ShibaDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("Shiba_Dividen_Tracker", "Shiba_Dividen_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**18); //must hold 10,000,000 at least tokens to get dividends
    }
